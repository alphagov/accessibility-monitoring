<?php
	 ...
	 *
	 * Modified for accessibility-monitoring  by JK, 2021-03-09
	 *
	...
	*/
	...
	foreach($service_blob as $service_provider => $service_list) {
			// looks for 'postgres' service
			if ($service_provider === 'postgres') {
			...
			foreach ($service_list as $some_service) {
					...
					if (in_array('postgres', $some_service['tags'], true)) {
							$pgsql_services[] = $some_service;
							continue;
					}
					// look for a service where the name includes 'a11ymon-postgres'
					if (strpos($some_service['name'], 'a11ymon-postgres') !== false) {
							$pgsql_services[] = $some_service;
					}
			}
	}
